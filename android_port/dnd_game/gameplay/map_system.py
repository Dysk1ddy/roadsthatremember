...
from ..data.id_aliases import DUNGEON_ID_ALIASES, MAP_NODE_ID_ALIASES, canonical_dungeon_id, canonical_map_node_id
...
ACT2_LEGACY_NODE_ALIASES = MAP_NODE_ID_ALIASES
ACT2_LEGACY_DUNGEON_ALIASES = DUNGEON_ID_ALIASES
...
class MapSystemMixin:
    ...
    def _ensure_map_state_payload(self) -> None:
        ...
        self._normalize_map_node_aliases(payload)
    ...
    def normalize_map_state_ids(self) -> None:
        if self.state is None:
            return
        if isinstance(self.state.flags.get(self.MAP_STATE_KEY), dict):
            self._ensure_map_state_payload()
        if isinstance(self.state.flags.get(self.ACT2_MAP_STATE_KEY), dict):
            self._ensure_act2_map_state_payload()
    ...
    def _normalize_map_node_aliases(self, payload: dict[str, Any]) -> None:
        current_node_id = payload.get("current_node_id")
        if isinstance(current_node_id, str):
            payload["current_node_id"] = canonical_map_node_id(current_node_id) or current_node_id
        current_dungeon_id = payload.get("current_dungeon_id")
        if isinstance(current_dungeon_id, str):
            payload["current_dungeon_id"] = canonical_dungeon_id(current_dungeon_id) or current_dungeon_id
        for key in ("visited_nodes", "node_history"):
            values = payload.get(key)
            if not isinstance(values, list):
                continue
            payload[key] = [canonical_map_node_id(value) or value for value in values]
    ...
    def _sync_story_beats_from_flags(self) -> None:
        ...
        if self.state.flags.get("iron_hollow_council_seen"):
            seen.add("stonehill_council")
        if self.state.flags.get("iron_hollow_after_watch_seen"):
    ...
    def _backfill_act1_visited_nodes(self, payload: dict[str, Any], current_node_id: str) -> None:
        ...
        if current_node_id == "greywake_road_breakout":
            ...
            return
        if current_node_id == "greywake_briefing":
            self._remember_act1_nodes(payload, *opening_path)
            return
        if current_node_id == "emberway_ambush":
            self._remember_act1_nodes(payload, *opening_path, "emberway_ambush")
            return

        # Reaching any Iron Hollow-era node means the opening road, briefing, and ambush already happened.
        self._remember_act1_nodes(payload, *opening_path, "emberway_ambush", "iron_hollow_hub")

        later_than_branches = {"ashfall_watch", "duskmere_manor", "emberhall_cellars"}
        later_than_ashfall = {"duskmere_manor", "emberhall_cellars"}

        if current_node_id == "blackglass_well" or current_node_id in later_than_branches or bool(self.state.flags.get("blackglass_well_cleared")):
            self._remember_act1_nodes(payload, "blackglass_well")
        if current_node_id == "red_mesa_hold" or current_node_id in later_than_branches or bool(self.state.flags.get("red_mesa_hold_cleared")):
            self._remember_act1_nodes(payload, "red_mesa_hold")
        ...
        if (
            ...
            or bool(self.state.flags.get("duskmere_revealed"))
            or bool(self.state.flags.get("duskmere_cleared"))
            or bool(self.state.flags.get("emberhall_revealed"))
        ):
            self._remember_act1_nodes(payload, "duskmere_manor")
    ...
    def _seed_act1_overworld_history(self, payload: dict[str, Any], current_node_id: str) -> None:
        ...
        elif current_node_id == "emberway_ambush":
            payload["node_history"] = ["greywake_briefing"]
        elif current_node_id == "iron_hollow_hub":
            payload["node_history"] = ["greywake_briefing", "emberway_ambush"]
    ...
    def return_to_iron_hollow(self, text: str) -> None:
        self.travel_to_act1_node("iron_hollow_hub", transition_text=text)
    ...
    def _act1_overworld_backtrack_allowed(self, current_node_id: str, candidate_node_id: str) -> bool:
        if current_node_id == "iron_hollow_hub" and candidate_node_id in ACT1_HIGH_ROAD_SIDE_BRANCH_NODE_IDS:
            return False
    ...
    def _act1_overworld_backtrack_text(self, from_node_id: str, to_node_id: str) -> str:
        ...
        if to_node_id == "greywake_briefing":
            return (
        ...
        if from_node_id == "iron_hollow_hub":
    ...
    def act1_room_navigation_options(self, dungeon: DungeonMap) -> list[tuple[str, str, str]]:
        ...
        if dungeon.exit_to_node_id == "road_decision_post_blackwake":
            ...
            options.append(("withdraw", "iron_hollow_hub", self.action_option("Withdraw to Iron Hollow")))
        return options
    ...
    def scene_iron_hollow_hub(self) -> None:
        assert self.state is not None
        self.act1_initialize_metrics()
        self._sync_map_state_with_scene(force_node_id="iron_hollow_hub")
        ...
        self.run_iron_hollow_council_event()
        ...
        while True:
            ...
            if not self.state.flags.get("blackglass_well_cleared"):
                label = self.action_option("Investigate Blackglass Well")
                if not self.can_visit_blackglass_well():
                    label = self.action_option("Investigate Blackglass Well (need a lead)")
                options.append(("blackglass_well", label))
            if not self.state.flags.get("red_mesa_hold_cleared"):
                label = self.action_option("Hunt the raiders at Red Mesa Hold")
                if not self.can_visit_red_mesa_hold():
                    label = self.action_option("Hunt the raiders at Red Mesa Hold (need a lead)")
                elif self.should_warn_for_red_mesa_hold():
                    label = self.action_option(
                        f"Hunt the raiders at Red Mesa Hold (recommended level {self.red_mesa_hold_recommended_level()})"
                    )
                options.append(("red_mesa", label))
            if (
            ...
            if selection_key == "steward":
                ...
                if not self.backtrack_act1_overworld_node():
                    ...
                    continue
                return
            elif selection_key == "blackglass_well":
                if not self.can_visit_blackglass_well():
                    self.say("You need a firmer lead on the well before marching the party into empty scrub.")
                    continue
                self.travel_to_act1_node("blackglass_well")
                return
            elif selection_key == "red_mesa":
                if not self.can_visit_red_mesa_hold():
                    self.say("You still need somebody to point you toward the tor and the raiders haunting it.")
                    continue
                if not self.confirm_red_mesa_hold_departure():
                    continue
                self.travel_to_act1_node("red_mesa_hold")
                return
                ...
                return
            elif selection_key == "duskmere":
                if not self.state.flags.get("duskmere_revealed"):
                    self.say("You need a firmer lead before committing to the buried manor routes.")
                    continue
                self.travel_to_act1_node("duskmere_manor")
                return
            else:
                self.travel_to_act1_node("emberhall_cellars")
                return

    def scene_blackglass_well(self) -> None:
        self.run_act1_dungeon("blackglass_well")
    ...
    def scene_road_decision_post_blackwake(self) -> None:
        ...
        options: list[tuple[str, str]] = [
            ("greywake", self.action_option("Return to Greywake with what you found.")),
        ...
        if selection_key == "greywake":
            ...
            self.state.flags["blackwake_return_destination"] = "greywake"
            self.speaker(
                "Mira Thann",
            ...
            if not self.state.flags.get("blackwake_greywake_reported"):
                self.state.flags["blackwake_greywake_reported"] = True
                ...
                if self.state.flags.get("greywake_private_room_intel") and not self.state.flags.get("greywake_contract_house_blackwake_reported"):
                    self.state.flags["greywake_contract_house_blackwake_reported"] = True
                    self.state.flags["greywake_contract_house_political_callback"] = True
    ...
    def scene_red_mesa_hold(self) -> None:
        self.run_act1_dungeon("red_mesa_hold")
    ...
    def scene_duskmere_manor(self) -> None:
        self.run_act1_dungeon("duskmere_manor")
    ...
    def scene_resonant_vault_outer_galleries(self) -> None:
        if self.state is not None:
            self.state.current_scene = "resonant_vault_outer_galleries"
        self.run_act2_dungeon("resonant_vault_outer_galleries")

    def scene_blackglass_causeway(self) -> None:
        if self.state is not None:
            self.state.current_scene = "blackglass_causeway"
        self.run_act2_dungeon("blackglass_causeway")
    ...
    def scene_meridian_forge(self) -> None:
        if self.state is not None:
            self.state.current_scene = "meridian_forge"
        self.run_act2_dungeon("meridian_forge")
    ...
    def run_act1_dungeon(self, node_id: str) -> None:
        ...
        while self.state is not None and self.state.current_scene == node.scene_key:
            ...
            if dungeon.exit_to_node_id == "road_decision_post_blackwake":
                ...
                return
            self.return_to_iron_hollow(f"You withdraw from {node.title} and ride back to Iron Hollow to regroup.")
            return
    ...
    def _run_act1_room(self, node_id: str, dungeon: DungeonMap, room: DungeonRoom) -> None:
        handlers = {
            ...
            ("blackglass_well", "well_ring"): self._blackglass_well_ring,
            ("blackglass_well", "salt_cart"): self._blackglass_well_salt_cart,
            ("blackglass_well", "supply_trench"): self._blackglass_well_supply_trench,
            ("blackglass_well", "gravecaller_lip"): self._blackglass_well_gravecaller_lip,
            ("red_mesa_hold", "goat_path"): self._red_mesa_goat_path,
            ("red_mesa_hold", "drover_hollow"): self._red_mesa_drover_hollow,
            ("red_mesa_hold", "shrine_ledge"): self._red_mesa_shrine_ledge,
            ("red_mesa_hold", "high_shelf"): self._red_mesa_high_shelf,
            ...
            ("duskmere_manor", "hidden_stair"): self._duskmere_hidden_stair,
            ("duskmere_manor", "cellar_intake"): self._duskmere_cellar_intake,
            ("duskmere_manor", "cistern_walk"): self._duskmere_cistern_walk,
            ("duskmere_manor", "cage_store"): self._duskmere_cage_store,
            ("duskmere_manor", "nothic_lair"): self._duskmere_nothic_lair,
    ...
    def _blackwake_ash_office(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if choice == 1:
            ...
            self.state.flags["blackwake_iron_hollow_pressure_clue"] = True
    ...
    def _run_act2_room(self, node_id: str, dungeon: DungeonMap, room: DungeonRoom) -> None:
        handlers = {
            ...
            ("resonant_vault_outer_galleries", "rail_junction"): self._resonant_vault_rail_junction,
            ("resonant_vault_outer_galleries", "slime_sluice"): self._resonant_vault_slime_sluice,
            ("resonant_vault_outer_galleries", "grimlock_side_run"): self._resonant_vault_grimlock_side_run,
            ("resonant_vault_outer_galleries", "collapsed_crane"): self._resonant_vault_collapsed_crane,
            ("resonant_vault_outer_galleries", "false_echo_loop"): self._resonant_vault_false_echo_loop,
            ("resonant_vault_outer_galleries", "deep_haul_gate"): self._resonant_vault_deep_haul_gate,
            ("blackglass_causeway", "causeway_lip"): self._blackglass_causeway_lip,
            ("blackglass_causeway", "drowned_shrine"): self._blackglass_drowned_shrine,
            ("blackglass_causeway", "choir_barracks"): self._blackglass_choir_barracks,
            ("blackglass_causeway", "anchor_chains"): self._blackglass_anchor_chains,
            ("blackglass_causeway", "blackwater_edge"): self._blackglass_blackwater_edge,
            ("blackglass_causeway", "far_landing"): self._blackglass_far_landing,
            ...
            ("meridian_forge", "forge_threshold"): self._forge_threshold,
            ("meridian_forge", "choir_pit"): self._forge_choir_pit,
            ("meridian_forge", "pact_anvil"): self._forge_pact_anvil,
            ("meridian_forge", "shard_channels"): self._forge_shard_channels,
            ("meridian_forge", "resonance_lens"): self._forge_resonance_lens,
            ("meridian_forge", "caldra_dais"): self._forge_caldra_dais,
        }
        handler = handlers[(node_id, room.room_id)]
        previous_context = getattr(self, "_post_combat_random_encounter_context", None)
    ...
    def _broken_prospect_broken_shelf(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if not self.has_quest("free_resonant_vault_captives"):
            self.grant_quest("free_resonant_vault_captives")
    ...
    def _resonant_vault_rail_junction(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        if not self.state.flags.get("resonant_vault_outer_seen"):
            self.say(
            ...
            self.state.flags["resonant_vault_outer_seen"] = True
        choice = self.scenario_choice(
    ...
    def _resonant_vault_slime_sluice(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        ...
        hero_bonus = self.apply_scene_companion_support("resonant_vault_outer_galleries")
    ...
    def _resonant_vault_grimlock_side_run(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def _resonant_vault_collapsed_crane(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def _resonant_vault_false_echo_loop(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def _resonant_vault_deep_haul_gate(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def _blackglass_causeway_lip(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        if not self.has_quest("sever_quiet_choir"):
            self.grant_quest("sever_quiet_choir")
        if not self.state.flags.get("blackglass_seen"):
            self.say(
            ...
            self.state.flags["blackglass_seen"] = True
        self._blackglass_apply_south_adit_payoff()
        self._blackglass_apply_hushfen_payoff()
        choice = self.scenario_choice(
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Religion", 14, context="to hear what sanctity still survives over the black water"):
                self.state.flags["blackglass_shrine_route_marked"] = True
            ...
            if self.skill_check(self.state.player, "Stealth", 14, context="to read the barracks watches without becoming part of the report"):
                self.state.flags["blackglass_barracks_watch_read"] = True
            ...
            if self.skill_check(self.state.player, "Athletics", 14, context="to feel where the old line will break before it throws you with it"):
                self.state.flags["blackglass_anchor_stress_read"] = True
                self.reward_party(xp=10, reason="reading the causeway's anchor strain")
        self.complete_act2_map_room(dungeon, room.room_id)

    def _blackglass_apply_south_adit_payoff(self) -> None:
        assert self.state is not None
        if self.state.flags.get("blackglass_south_adit_payoff_applied"):
            return
        outcome = str(self.state.flags.get("act2_captive_outcome", "uncertain"))
        if outcome == "many_saved":
            self.state.flags["blackglass_south_adit_payoff_applied"] = True
            self.state.flags["blackglass_survivor_testimony"] = True
            self.state.flags["blackglass_barracks_watch_read"] = True
            self.say(
        ...
        if outcome != "few_saved":
            return
        self.state.flags["blackglass_south_adit_payoff_applied"] = True
        self.state.flags["blackglass_choir_reserve_intact"] = True
        self.say(
    ...
    def _blackglass_apply_hushfen_payoff(self) -> None:
        assert self.state is not None
        if not self.state.flags.get("hushfen_chapel_relit"):
            return
        if not self.state.flags.get("blackglass_hushfen_lamp_guidance"):
            self.state.flags["blackglass_hushfen_lamp_guidance"] = True
            self.state.flags["blackglass_shrine_route_marked"] = True
            self.say(
        ...
        self.state.flags["blackglass_hushfen_pressure_payoff"] = True
        self.act2_shift_metric(
            "act2_whisper_pressure",
            -1,
    ...
    def _blackglass_drowned_shrine(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        self.say(
        ...
        dc = 13 if self.state.flags.get("blackglass_shrine_route_marked") or self.state.flags.get("pale_witness_truth_clear") else 14
        choice = self.scenario_choice(
        ...
        if shrine_bonus:
            self.state.flags["blackglass_shrine_sanctity_named"] = True
    ...
    def _blackglass_choir_barracks(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        enemies = [create_enemy("cult_lookout"), create_enemy("choir_adept")]
        if self.state.flags.get("blackglass_choir_reserve_intact"):
        ...
        hero_bonus = self.apply_scene_companion_support("blackglass_causeway")
        if self.state.flags.get("blackglass_survivor_testimony"):
            self.say("A rescued South Adit witness mouths the blind-side count as the barracks settles into it.")
        if self.state.flags.get("blackglass_barracks_watch_read"):
            hero_bonus += 1
        ...
        if selection_key == "stealth":
            ...
            if self.skill_check(self.state.player, "Investigation", 14, context="to seize the barracks orders intact"):
                hero_bonus += 1
                self.state.flags["blackglass_barracks_orders_taken"] = True
    ...
    def _blackglass_anchor_chains(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        self.say(
        ...
        dc = 13 if self.state.flags.get("blackglass_anchor_stress_read") else 14
        choice = self.scenario_choice(
        ...
        if choice == 1:
            ...
            if clean_sabotage:
                self.state.flags["blackglass_anchor_weak_point_found"] = True
        else:
            ...
            if clean_sabotage:
                self.state.flags["blackglass_causeway_footing_marked"] = True
        self.complete_act2_map_room(dungeon, room.room_id)
        if clean_sabotage:
    ...
    def _blackglass_blackwater_edge(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        ...
        if self.act2_metric_value("act2_whisper_pressure") >= 4:
            ...
            enemies.append(self.act2_pick_enemy(("starblighted_miner", "blacklake_pincerling", "cult_lookout")))
        hero_bonus = self.apply_scene_companion_support("blackglass_causeway")
        if self.state.flags.get("blackglass_shrine_purified"):
            hero_bonus += 1
            self.apply_status(self.state.player, "blessed", 1, source="the Blackglass shrine")
        if self.state.flags.get("blackglass_barracks_raided"):
            hero_bonus += 1
        if self.state.flags.get("blackglass_causeway_shaken"):
            hero_bonus += 1
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Survival", 14, context="to guide the party across the safe blackwater footing"):
                hero_bonus += 2
                self.state.flags["blackglass_waterline_read"] = True
        elif choice == 2:
    ...
    def _blackglass_far_landing(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        ...
        elif not self.state.flags.get("blackglass_barracks_raided"):
        ...
        if self.state.flags.get("blackglass_barracks_orders_taken"):
            hero_bonus += 1
            enemies[1].current_hp = max(1, enemies[1].current_hp - 4)
        if self.state.flags.get("blackglass_causeway_footing_marked") or self.state.flags.get("blackglass_waterline_read"):
            hero_bonus += 1
        choice = self.scenario_choice(
        ...
        self.act2_award_milestone_gear(
            "act2_blackglass_milestone_gear",
            self.act2_blackglass_milestone_item(),
    ...
    def _blackglass_relay_keeper_ledger(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        dc = 13 if self.state.flags.get("blackglass_barracks_orders_taken") or self.state.flags.get("blackglass_relay_gate_route_marked") else 14
        choice = self.scenario_choice(
    ...
    def _forge_threshold(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if not self.state.flags.get("forge_seen"):
            ...
            elif self.state.flags.get("blackglass_barracks_raided"):
        ...
        if (
            ...
            or self.state.flags.get("blackglass_causeway_shaken")
            or self.state.flags.get("blackglass_anchor_weak_point_found")
        ...
        if choice == 1:
            dc = 13 if self.state.flags.get("blackglass_barracks_orders_taken") or self.state.flags.get("forge_reserve_timing_known") else 14
            ...
            dc = 13 if self.state.flags.get("blackglass_shrine_purified") else 14
    ...
    def _forge_choir_pit(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        full_party = len(self.state.party_members()) >= 4
        enemies = [create_enemy("choir_adept"), create_enemy("cult_lookout")]
        if not self.state.flags.get("blackglass_barracks_raided") and not self.state.flags.get("forge_signal_grounded"):
        ...
        hero_bonus = self.apply_scene_companion_support("meridian_forge")
        if self.state.flags.get("blackglass_barracks_raided"):
            hero_bonus += 1
        if self.state.flags.get("forge_signal_grounded"):
            hero_bonus += 1
            self.apply_status(enemies[0], "reeling", 1, source="the Blackglass relay line dying before it reaches the pit")
        if self.state.flags.get("blackglass_barracks_orders_taken") or self.state.flags.get("forge_threshold_orders_read"):
            hero_bonus += 1
    ...
    def _forge_pact_anvil(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if self.state.flags.get("nim_countermeasure_notes"):
            dc -= 1
        if self.state.flags.get("blackglass_shrine_purified") or self.state.flags.get("forge_threshold_sanctified"):
            dc -= 1
        choice = self.scenario_choice(
    ...
    def _forge_shard_channels(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if self.state.flags.get("blackglass_anchor_weak_point_found") or self.state.flags.get("forge_threshold_shard_timing"):
            hero_bonus += 1
        if self.state.flags.get("forge_signal_grounded"):
            hero_bonus += 1
            enemies[0].current_hp = max(1, enemies[0].current_hp - 4)
        if self.state.flags.get("blackglass_causeway_footing_marked"):
            hero_bonus += 1
        choice = self.scenario_choice(
    ...
    def _forge_resonance_lens(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if self.state.flags.get("blackglass_barracks_orders_taken"):
            dc -= 1
        if self.state.flags.get("blackglass_causeway_shaken") or self.state.flags.get("blackglass_anchor_weak_point_found"):
            dc -= 1
    ...
    def _forge_caldra_dais(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if self.state.flags.get("blackglass_barracks_raided") and full_party and (hard_route or high_pressure):
            enemies.append(self.act2_pick_enemy(("memory_taker_adept", "forge_echo_stalker")))
        elif not self.state.flags.get("blackglass_barracks_raided") and not self.state.flags.get("forge_signal_grounded"):
        ...
        if self.state.flags.get("blackglass_shrine_purified") or self.state.flags.get("forge_threshold_sanctified"):
            ...
            hero_bonus += 1
            parley_dc -= 1
        if self.state.flags.get("blackglass_barracks_orders_taken") or self.state.flags.get("forge_lens_support_line_named"):
            hero_bonus += 1
            if len(enemies) > 1:
                enemies[1].current_hp = max(1, enemies[1].current_hp - 4)
        if self.state.flags.get("forge_signal_grounded"):
        ...
        if self.state.flags.get("blackglass_causeway_shaken") or self.state.flags.get("forge_shard_channels_disrupted"):
            hero_bonus += 1
            if len(enemies) > 1:
    ...
    def _blackglass_well_ring(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        if not self.state.flags.get("blackglass_well_seen"):
            self.say(
            ...
            self.state.flags["blackglass_well_seen"] = True
        ...
        hero_bonus = self.apply_scene_companion_support("blackglass_well")
        choice = self.scenario_choice(
        ...
        if choice == 1:
            ...
            if success:
                ...
                if followup == 1:
                    ...
                    self.state.flags["blackglass_well_ritual_sabotaged"] = True
                    ...
                    self.state.flags["blackglass_well_sentry_picked"] = True
                    sentry = enemies[-1]
                    ...
                    self.state.flags["blackglass_well_deeper_infiltration"] = True
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You fall back from the well before the site can swallow the whole party.")
            return
    ...
    def _blackglass_well_salt_cart(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def _blackglass_well_supply_trench(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def is_blackglass_well_vaelith_encounter(self, encounter) -> bool:
        return (
            getattr(encounter, "title", "") == "Miniboss: Vaelith Marr"
            and self.state is not None
            and self.state.current_scene == "blackglass_well"
        )
    ...
    def is_duskmere_cistern_eye_encounter(self, encounter) -> bool:
        return (
            getattr(encounter, "title", "") == "The Cistern Eye"
            and self.state is not None
            and self.state.current_scene == "duskmere_manor"
        )

    def is_duskmere_cellar_encounter(self, encounter) -> bool:
        return (
    ...
    def is_duskmere_manor_encounter(self, encounter) -> bool:
        return (
    ...
    def on_encounter_round_start(self, encounter, heroes, enemies, initiative, round_number: int) -> None:
        self.handle_blackglass_well_vaelith_gravecall(encounter, heroes, enemies, initiative, round_number)
        self.handle_blackglass_well_vaelith_ritual_terrain(encounter, heroes, enemies)
        ...
        self.handle_duskmere_cellar_alarm_chain(encounter, enemies, initiative, round_number)
        self.handle_duskmere_manor_collapse_timer(encounter, heroes, enemies, round_number)
    ...
    def handle_blackglass_well_vaelith_gravecall(self, encounter, heroes, enemies, initiative, round_number: int) -> None:
        if not self.is_blackglass_well_vaelith_encounter(encounter):
            return
    ...
    def handle_blackglass_well_vaelith_ritual_terrain(self, encounter, heroes, enemies) -> None:
        if not self.is_blackglass_well_vaelith_encounter(encounter):
            return
        vaelith = self.find_vaelith_marr(enemies)
        if vaelith is None or not vaelith.is_conscious():
        ...
        if not conscious_heroes:
            return
        target = max(conscious_heroes, key=self.blackglass_well_damage_pressure_score)
    ...
    def handle_cistern_eye_secret_tax(self, encounter, heroes, enemies, round_number: int) -> None:
        if not self.is_duskmere_cistern_eye_encounter(encounter) or round_number != 2:
            return
        if self.state is not None and self.state.flags.get("duskmere_eye_read"):
            return
    ...
    def duskmere_cleared_room_count(self) -> int:
        if self.state is None or self.state.current_scene != "duskmere_manor":
            return 0
        self._sync_map_state_with_scene()
        payload = self._map_state_payload()
        cleared_rooms = set(payload["cleared_rooms"])
        dungeon_id = payload.get("current_dungeon_id")
        if dungeon_id == "duskmere_undercellars":
    ...
    def handle_duskmere_cellar_alarm_chain(self, encounter, enemies, initiative, round_number: int) -> None:
        if not self.is_duskmere_cellar_encounter(encounter) or round_number != 3:
            return
        if self.state is None or not self.state.flags.get("duskmere_entry_approach_failed"):
            return
        marker = enemies[0] if enemies else None
        if marker is None or marker.bond_flags.get("duskmere_cellar_alarm_chain_triggered"):
            return
        marker.bond_flags["duskmere_cellar_alarm_chain_triggered"] = True
    ...
    def handle_duskmere_manor_collapse_timer(self, encounter, heroes, enemies, round_number: int) -> None:
        if not self.is_duskmere_manor_encounter(encounter) or round_number != 3:
            return
        if self.duskmere_cleared_room_count() < 2:
            return
        marker = next((enemy for enemy in enemies if enemy.is_conscious()), None)
        if marker is None or marker.bond_flags.get("duskmere_collapse_timer_triggered"):
            return
        marker.bond_flags["duskmere_collapse_timer_triggered"] = True
        targets = []
        conscious_heroes = [hero for hero in heroes if hero.is_conscious()]
        conscious_enemies = [enemy for enemy in enemies if enemy.is_conscious()]
        if conscious_heroes:
        ...
        for target in targets:
            self.resolve_duskmere_falling_stone(target)

    def resolve_duskmere_falling_stone(self, target) -> None:
    ...
    def handle_cistern_eye_secret_hunger(self, encounter, heroes, enemies, round_number: int) -> None:
        if not self.is_duskmere_cistern_eye_encounter(encounter) or round_number % 3 != 0:
            return
        eye = self.find_cistern_eye(enemies)
    ...
    def handle_vaelith_bloodied_ward(self, encounter, target, previous_hp: int) -> None:
        ...
        if not heroes:
            return
        target_hero = max(heroes, key=self.blackglass_well_damage_pressure_score)
    ...
    def blackglass_well_damage_pressure_score(self, hero) -> tuple[int, int, int]:
        return self.combat_damage_pressure_score(hero)

    def _blackglass_well_gravecaller_lip(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        self.say(
        ...
        if not (self.state.flags.get("blackglass_well_prospector_rescued") or self.state.flags.get("blackglass_well_notes_found")):
        ...
        boss_bonus = int(self.state.flags.get("blackglass_well_prospector_rescued", False)) + int(self.state.flags.get("blackglass_well_notes_found", False))
        if self.state.flags.get("blackglass_well_ritual_sabotaged"):
        ...
        if self.state.flags.get("blackglass_well_sentry_picked") and len(boss_enemies) > 1:
        ...
        if self.state.flags.get("blackglass_well_deeper_infiltration"):
            boss_bonus += 1
        choice = self.scenario_choice(
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You break contact and retreat to Iron Hollow with the well still active behind you.")
            return
        ...
        self.return_to_iron_hollow("Blackglass Well falls quiet behind you as the road back to Iron Hollow opens again.")

    def _red_mesa_goat_path(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        if not self.state.flags.get("red_mesa_hold_seen"):
            self.say(
            ...
            self.state.flags["red_mesa_hold_seen"] = True
        party_size = self.act1_party_size()
        ...
        hero_bonus = self.apply_scene_companion_support("red_mesa_hold")
        choice = self.scenario_choice(
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You break away from the tor and retreat before the shelf turns into a killing bowl.")
            return
    ...
    def _red_mesa_drover_hollow(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        ...
        if followup == 1:
            ...
            self.state.flags["red_mesa_spotter_signal"] = True
            ...
            self.state.flags["red_mesa_beast_stampede"] = True
    ...
    def _red_mesa_shrine_ledge(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        ...
        if choice == 1:
            ...
            self.state.flags["red_mesa_rhogar_omen"] = True
    ...
    def _red_mesa_high_shelf(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        self.say(
        ...
        boss_bonus = int(self.state.flags.get("red_mesa_drover_rescued", False)) + int(self.state.flags.get("red_mesa_shrine_secured", False))
        if self.state.flags.get("red_mesa_spotter_signal"):
            boss_bonus += 1
            self.say("A sharp whistle from below the shelf warns you exactly when Brughor commits his weight forward.")
        if self.state.flags.get("red_mesa_beast_stampede") and len(boss_enemies) > 1:
        ...
        if self.state.flags.get("red_mesa_rhogar_omen"):
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You pull clear of the upper shelf and retreat to Iron Hollow to regroup.")
            return
        ...
        self.return_to_iron_hollow("Red Mesa Hold falls behind you as the ridge wind finally goes clean.")
    ...
    def _cinderfall_collapsed_gate(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You break away from Cinderfall before the relay line can close around you.")
            return
    ...
    def _cinderfall_ember_relay(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You pull out of Cinderfall before the relay crew can pin you in the ruin.")
            return
        ...
        self.return_to_iron_hollow("Cinderfall goes dark behind you. Whatever waits at Ashfall will now be doing it with thinner reserves and worse timing.")
    ...
    def _ashfall_breach_gate(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You fall back to Iron Hollow to rethink the assault.")
            return
    ...
    def _ashfall_lower_barracks(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You drag the party clear of the lower yard and retreat before the courtyard can close on you.")
            return
    ...
    def _ashfall_rukhar_command(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You escape the watchtower and retreat to Iron Hollow to regroup.")
            return
        ...
        self.return_to_iron_hollow("Ashfall Watch breaks under the assault, and the road home finally opens.")

    def _duskmere_hidden_stair(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        self.say(
        ...
        if choice == 1:
            ...
            self.state.flags["duskmere_hidden_entry"] = success
            self.state.flags["duskmere_entry_approach_failed"] = not success
            if success:
            ...
            self.state.flags["duskmere_chapel_entry"] = success
            self.state.flags["duskmere_entry_approach_failed"] = not success
            if success:
            ...
            self.state.flags["duskmere_cistern_breach"] = success
            self.state.flags["duskmere_entry_approach_failed"] = not success
            if success:
    ...
    def _duskmere_cellar_intake(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        party_size = self.act1_party_size()
        ...
        hero_bonus = self.apply_scene_companion_support("duskmere_manor")
        if self.state.flags.pop("duskmere_hidden_entry", False):
            self.apply_status(enemies[0], "surprised", 1, source="your hidden entry")
            hero_bonus += 2
        if self.state.flags.pop("duskmere_chapel_entry", False):
            enemies[1].current_hp = max(1, enemies[1].current_hp - 4)
            self.apply_status(enemies[1], "reeling", 1, source="your chapel-side opening")
            hero_bonus += 1
        if self.state.flags.pop("duskmere_cistern_breach", False):
            self.apply_status(self.state.player, "emboldened", 2, source="a brutal manor breach")
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You pull back from the manor tunnels before the whole cellar network can close around you.")
            return

        self.complete_map_room(dungeon, room.room_id)
        self.state.flags.pop("duskmere_entry_approach_failed", None)
    ...
    def _duskmere_cistern_walk(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        ...
        if choice == 1:
            ...
            if success:
                self.state.flags["duskmere_eye_read"] = True
            ...
            self.state.flags["duskmere_eye_ambushed"] = True
            self.say("The sack hits water, the yellow eye turns, and you use the half-second it gives you.")

        self.complete_map_room(dungeon, room.room_id)

    def _duskmere_cage_store(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
    ...
    def _duskmere_nothic_background_read(self) -> None:
        assert self.state is not None
        background_reads = {
        ...
        self.state.flags["duskmere_nothic_background_read"] = background
        self.speaker(
            "Cistern Eye",
            background_reads.get(background, "You came south with one story on your tongue and another locked behind your teeth."),
        )

    def _active_duskmere_secret_companion(self, name: str):
        assert self.state is not None
        companion = self.find_companion(name)
        return companion if companion in self.state.companions else None

    def _record_duskmere_nothic_trade_info(self) -> None:
        assert self.state is not None
        if not self.state.flags.get("duskmere_nothic_trade_info_gained"):
            self.state.flags["deep_ledger_hint_count"] = int(self.state.flags.get("deep_ledger_hint_count", 0) or 0) + 1
        self.state.flags["duskmere_nothic_trade_info_gained"] = True
    ...
    def _record_duskmere_nothic_cinderfall_lore(self) -> None:
        assert self.state is not None
        if not self.state.flags.get("duskmere_nothic_cinderfall_lore"):
            self.state.flags["deep_ledger_hint_count"] = int(self.state.flags.get("deep_ledger_hint_count", 0) or 0) + 1
        self.state.flags["duskmere_nothic_cinderfall_lore"] = True
    ...
    def _record_duskmere_nothic_resonant_vault_lore(self) -> None:
        assert self.state is not None
        if not self.state.flags.get("duskmere_nothic_resonant_vault_lore"):
            self.state.flags["deep_ledger_hint_count"] = int(self.state.flags.get("deep_ledger_hint_count", 0) or 0) + 1
        self.state.flags["duskmere_nothic_resonant_vault_lore"] = True
        self.add_clue(
    ...
    def _duskmere_adjust_active_companions_for_bargain(self, delta: int, reason: str) -> None:
        assert self.state is not None
    ...
    def _duskmere_nothic_trade_secret(self) -> int:
        assert self.state is not None
        options: list[tuple[str, str]] = [
        ...
        bryn = self._active_duskmere_secret_companion("Bryn Underbough")
        if bryn is not None:
            options.append(
                (
                    "betray_bryn",
        ...
        rhogar = self._active_duskmere_secret_companion("Rhogar Valeguard")
        if rhogar is not None:
            options.append(
                (
                    "betray_rhogar",
        ...
        self.state.flags["duskmere_nothic_trade"] = trade_key
        self.state.flags["duskmere_nothic_trade_paid"] = True

        if trade_key == "memory":
            self.player_action("Give it a memory from your own past.")
            self.state.flags["duskmere_nothic_memory_paid"] = True
            ...
            self._record_duskmere_nothic_trade_info()
            return 0

        if trade_key == "self_truth":
            self.player_speaker("The truth I keep walking around is mine to speak.")
            self.state.flags["duskmere_nothic_self_truth_spoken"] = True
    ...
    def _duskmere_nothic_deep_bargain(self) -> tuple[int, int]:
        assert self.state is not None
        enemy_bonus = 0
        self.state.flags["duskmere_nothic_bargain_tier"] = 1
        self.state.flags["duskmere_nothic_sanity_cost"] = 1
        self._record_duskmere_nothic_trade_info()
        ...
        self.state.flags["duskmere_nothic_bargain_tier"] = 2
        self.state.flags["duskmere_nothic_sanity_cost"] = 2
        self._record_duskmere_nothic_cinderfall_lore()
        self.apply_status(self.state.player, "frightened", 1, source="the Cistern Eye naming Cinderfall")
        enemy_bonus += 1
        self._duskmere_adjust_active_companions_for_bargain(-1, "you kept feeding the Cistern Eye for more secrets")
        choice = self.scenario_choice(
        ...
        self.state.flags["duskmere_nothic_bargain_tier"] = 3
        self.state.flags["duskmere_nothic_sanity_cost"] = 3
        self.state.flags["duskmere_nothic_bargain_whispered_through"] = True
        self._record_duskmere_nothic_resonant_vault_lore()
        ...
        self._duskmere_adjust_active_companions_for_bargain(-1, "you let the Cistern Eye press past useful truth into wrongness")
        enemy_bonus += 2
        return 0, enemy_bonus

    def _duskmere_nothic_route_shell(self, nothic) -> tuple[int, int]:
        assert self.state is not None
        self.say(
        ...
        self._duskmere_nothic_background_read()
        choice = self.scenario_choice(
        ...
        enemy_bonus = 0
        if choice == 1:
            self.state.flags["duskmere_nothic_route"] = "kill"
            self.player_action("Kill it before it pries any deeper.")
            tolan = self.find_companion("Tolan Ironshield")
            if tolan in self.state.companions and not self.state.flags.get("duskmere_nothic_tolan_kill_approved"):
                self.state.flags["duskmere_nothic_tolan_kill_approved"] = True
            ...
            self.state.flags["duskmere_nothic_route"] = "trade"
            ...
            bargain_hero_bonus, bargain_enemy_bonus = self._duskmere_nothic_deep_bargain()
            ...
            self.state.flags["duskmere_nothic_route"] = "deceive"
            ...
            if success:
                self.state.flags["duskmere_nothic_deceived"] = True
                self._record_duskmere_nothic_trade_info()
                self._record_duskmere_nothic_cinderfall_lore()
                self._record_duskmere_nothic_resonant_vault_lore()
                ...
                self.state.flags["duskmere_nothic_deception_failed"] = True
    ...
    def _duskmere_nothic_lair(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        assert self.state is not None
        party_size = self.act1_party_size()
        ...
        boss_bonus = int(self.state.flags.get("duskmere_eye_read", False)) + (2 if self.state.flags.get("duskmere_eye_ambushed", False) else 0)
        if not self.state.flags.get("duskmere_records_secured"):
        ...
        route_bonus, enemy_bonus = self._duskmere_nothic_route_shell(second_enemies[0])
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You retreat from the cistern dark before the manor can claim the rest of the night.")
            return
        ...
        if not self.state.flags.get("duskmere_records_secured"):
            self.add_inventory_item("scroll_arcane_refresh", source="a sealed coffer in the cistern alcove")
        self.return_to_iron_hollow("The cistern goes still, and the buried route beneath Duskmere finally breaks open.")
    ...
    def _emberhall_antechamber(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You retreat to the surface before the last chamber can close around you.")
            return
    ...
    def _emberhall_black_reserve(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You pull back from the reserve room before it can close around you.")
            return
    ...
    def _emberhall_varyn_sanctum(self, dungeon: DungeonMap, room: DungeonRoom) -> None:
        ...
        if outcome == "defeat":
            ...
            return
        if outcome == "fled":
            self.return_to_iron_hollow("You escape the cellars and return to the surface to recover.")
            return
