...
from ..data.id_aliases import FLAG_ID_ALIASES, canonical_flag_id
...
class StoryAct2ScaffoldMixin:
    ...
    ACT2_LEGACY_FLAG_ALIASES = tuple(FLAG_ID_ALIASES.items())
    ACT2_BRANCH_FLAGS = (
    ...
    def normalize_act2_legacy_flags(self) -> None:
        ...
        for key in ("act2_neglected_lead",):
            ...
            if isinstance(value, str):
                self.state.flags[key] = canonical_flag_id(value) or value
    ...
    def act2_blackglass_milestone_item(self) -> str:
        assert self.state is not None
        if self.state.flags.get("blackglass_shrine_purified") or self.act2_metric_value("act2_whisper_pressure") >= 4:
    ...
    def act2_initialize_metrics(self, *, force: bool = False) -> None:
        ...
        if self.state.flags.get("quest_reward_elira_mercy_blessing"):
            town += 1
            whisper -= 1
        if self.state.flags.get("greywake_contract_house_political_callback"):
            route += 1
    ...
    def act2_initialize_blackwake_callbacks(self) -> None:
        assert self.state is not None
        if self.state.flags.get("greywake_contract_house_political_callback"):
            self.state.flags["act2_greywake_witness_pressure_active"] = True
            if not self.state.flags.get("act2_greywake_witness_callback_recorded"):
                self.state.flags["act2_greywake_witness_callback_recorded"] = True
                self.add_journal(
    ...
    def act2_campaign_focus_lines(self) -> list[str]:
        ...
        if self.state.flags.get("resonant_vault_outer_cleared"):
        ...
        if self.state.flags.get("blackglass_crossed"):
            route_parts.append("the Blackglass threshold is open")
        elif self.state.flags.get("blackglass_shrine_purified") or self.state.flags.get("blackglass_barracks_raided"):
        ...
        if self.state.flags.get("act2_greywake_witness_pressure_active"):
            lines.append(
        ...
        if (
            ...
            or self.state.flags.get("blackglass_barracks_orders_taken")
        ):
    ...
    def scene_broken_prospect(self) -> None:
        ...
        if outcome == "fled":
            ...
            return
        self.state.flags["resonant_vault_reached"] = True
    ...
    def scene_resonant_vault_outer_galleries(self) -> None:
        ...
        if outcome == "fled":
            ...
            return
        self.state.flags["resonant_vault_outer_cleared"] = True
    ...
    def scene_blackglass_causeway(self) -> None:
        ...
        self.run_dialogue_input("act2_blackglass_entry", max_entries=2)
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Religion", 14, context="to wake the shrine before the Choir notices what you are doing"):
                hero_bonus += 1
                self.state.flags["blackglass_shrine_purified"] = True
            ...
            if self.skill_check(self.state.player, "Stealth", 14, context="to gut the barracks without losing the crossing"):
                hero_bonus += 2
                self.state.flags["blackglass_barracks_raided"] = True
                self.act2_shift_metric(
                    "act2_route_control",
                    1,
            ...
            if self.skill_check(self.state.player, "Athletics", 14, context="to break the line's stability without dropping your own company with it"):
                hero_bonus += 2
                self.state.flags["blackglass_causeway_shaken"] = True
        ...
        if outcome == "fled":
            ...
            return
        self.state.flags["blackglass_crossed"] = True
