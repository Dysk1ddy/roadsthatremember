...
class StoryAct1ExpandedMixin:
    ...
    def act1_side_paths_cleared(self) -> bool:
        assert self.state is not None
        return bool(
            self.state.flags.get("blackglass_well_cleared") and self.state.flags.get("red_mesa_hold_cleared")
        )

    def can_visit_blackglass_well(self) -> bool:
        assert self.state is not None
        return bool(self.has_quest("silence_blackglass_well") or self.state.flags.get("miners_exchange_lead"))

    def can_visit_red_mesa_hold(self) -> bool:
        assert self.state is not None
        return bool(self.has_quest("break_red_mesa_raiders") or self.state.flags.get("edermath_orchard_lead"))

    def has_edermath_orchard_interactions(self) -> bool:
        assert self.state is not None
        return bool(
            not self.state.flags.get("edermath_orchard_seen")
            or self.quest_is_ready("break_red_mesa_raiders")
            or not self.state.flags.get("edermath_orchard_blight_checked")
            or not self.state.flags.get("edermath_orchard_red_mesa_hold_asked")
    ...
    def has_miners_exchange_interactions(self) -> bool:
        ...
        return bool(
            ...
            or self.quest_is_ready("silence_blackglass_well")
    ...
    def red_mesa_hold_recommended_level(self) -> int:
        return 3

    def should_warn_for_red_mesa_hold(self) -> bool:
        assert self.state is not None
        return self.state.player.level < self.red_mesa_hold_recommended_level()

    def confirm_red_mesa_hold_departure(self) -> bool:
        if not self.should_warn_for_red_mesa_hold():
            return True
        recommended_level = self.red_mesa_hold_recommended_level()
        self.say(
    ...
    def run_iron_hollow_council_event(self) -> None:
        assert self.state is not None
        if self.state.flags.get("iron_hollow_council_seen") or not self.act1_side_paths_cleared():
            return
        ...
        if choice == 1:
            ...
            if success:
                self.say(
                    "The pattern is obvious once you hear it: prisoners and orders matter more to Rukhar than plunder, which means the lower barracks and map room are the command nerve."
                )
        ...
        self.state.flags["iron_hollow_council_seen"] = True

    def run_after_watch_gathering(self) -> None:
        assert self.state is not None
        if self.state.flags.get("iron_hollow_after_watch_seen") or not self.state.flags.get("ashfall_watch_cleared"):
            return
        ...
        self.state.flags["duskmere_revealed"] = True
        ...
        while True:
            ...
            if selection_key == "steward":
                ...
                if not self.state.flags.get("duskmere_revealed"):
                    ...
                    continue
                self.state.current_scene = "duskmere_manor"
                return
            else:
    ...
    def visit_edermath_orchard(self) -> None:
        ...
        while True:
            options: list[tuple[str, str]] = []
            if self.quest_is_ready("break_red_mesa_raiders"):
            ...
            if not self.state.flags.get("edermath_orchard_red_mesa_hold_asked"):
            ...
            if selection_key == "turn_in":
                ...
                self.turn_in_quest("break_red_mesa_raiders", giver="Daran Orchard")
                ...
                self.state.flags["edermath_orchard_red_mesa_hold_asked"] = True
    ...
    def run_edermath_old_cache_scene(self) -> None:
        ...
        if self.state.flags.get("red_mesa_hold_cleared"):
            self.speaker(
                "Daran Orchard",
    ...
    def visit_miners_exchange(self) -> None:
        ...
        while True:
            options: list[tuple[str, str]] = []
            if self.quest_is_ready("silence_blackglass_well"):
            ...
            if selection_key == "turn_in":
                ...
                self.turn_in_quest("silence_blackglass_well", giver="Halia Vey")
    ...
    def scene_blackglass_well(self) -> None:
        ...
        self.state.flags["blackglass_well_cleared"] = True
    ...
    def scene_red_mesa_hold(self) -> None:
        ...
        self.state.flags["red_mesa_hold_cleared"] = True
    ...
    def scene_duskmere_manor(self) -> None:
        ...
        enemy_bonus = 0
        route_shell = getattr(self, "_duskmere_nothic_route_shell", None)
        ...
        self.state.flags["duskmere_cleared"] = True
